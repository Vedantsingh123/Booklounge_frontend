...
const SignUp = () => {
  ...
  const submit = async () => {
    try{
      if(Values.username === "" || Values.email === "" || Values.password === "" || Values.address === ""){
        ...
        const response = await axios.post('https://bookloune-backend.onrender.com/userapi/Sign-Up',Values);
  ...
  return (
    ...
    px-12 py-8 flex items-center justify-center'>
      ...
      <div className='mt-4'>
       <button className='w-full bg-yellow-500 text-white font-semibold py-2 rounded hover:bg-white hover:text-zinc-900 transition-all duration-300' onClick={submit}>SignUp</button>
       <div> 
        <p className='flex mt-4 items-center justify-center text-zinc-200 font-semibold'>Or</p>
        <p className='flex mt-4 items-center justify-center text-zinc-500 font-semibold'>Already have an account? &nbsp; <Link to='/LogIn' className='hover:text-blue-500'><p>LogIn</p></Link></p>
       </div>
      </div>
      </div>
      </div>
